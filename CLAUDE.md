...
5. Write one markdown file per input CSV via `DataFrame.to_markdown(index=False)`, named from the first row's `Account Name` / `Account Number`. When `output.markdown.summary_header` is enabled in the contract, prepend an `**As of:**` line (extracted from the Fidelity `Date downloaded` footer) and one `**<label>:** $X,XXX.XX` line per configured total (default `Total Current Value`, summed from the `Current value` column). The total is a sum of the export's column values, not a Fidelity-quoted account balance. The script does not split multi-account CSVs — callers must export one account per CSV.
